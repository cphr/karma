...
final String rankAttribute = "rank";
 ...
 public Value dbnodes(final Value value,QueryContext queryContext) {
...
{           
	...
	try
	{
              ...
              String DB = new CreateDB("Collection", org.karma.helper.projectInfo.DoxygenPath ).execute(context);
              String xpathExpression = "doc("+"file+"+")//compounddef/sectiondef/memberdef";
              
              //SAX
              SAXReader reader = new SAXReader();
              Document document = reader.read(file);
              System.out.println(reference+"<----");
              //<ul>{for $x in doc("index.xml")/doxygenindex/compound/member/name order by $x return <li>{$x}</li>}</ul>
              @SuppressWarnings("unchecked")
              List<Node> nodes = document.selectNodes(xpathExpression);	        
              
              String stringReturn = new XQuery(xpathExpression).execute(context);
              QueryProcessor qp = new QueryProcessor(xpathExpression,context);
              Value result = qp.value();
              
              
              //List<Node> nodes = new XQuery(xpathExpression).execute(context); //document.selectNodes(xpathExpression);		
              // Value value = nodes 
              // List<Node> nodes = dbnodes(value,null);
