...
AUTHOR:         Monty Zukowski (jamz@cdsnet.net) April 28, 1998
		     ...
		     Hardly modified by Massimo Trubia(massimotrubia83@gmail.com), 
					      Elena Carobene(elecar@gmail.com), 
					      Alessio Tognazzolo(alessio.tognazzolo@gmail.com)
             	      for realizing a C cross compiler for EduMIPS64 simulator called EduXcomp
...

{
    ...
    public int getLineNumber(String stringa){
      int index=stringa.indexOf("line:");
      String sottostringa=stringa.substring(index, stringa.length());
      String[] splitted=sottostringa.split(" ");
      String valore= splitted[0].substring(5, splitted[0].length());
...
initDecl[AST declarationSpecifiers]
        ...
        :       declName = d:declarator[false]
                                        {   AST ds1, d1;
                                            ...
                                            int nnum=getLineNumber(stringa);
    
                                           if(symbolTable.lookupNameInCurrentScope(declName)!=null)
                                              throw new SemanticException("Identificatore "+ #declName+" gia' presente",getFilename(),nnum); //in caso non personalizzare troppo
...
statement
        ...
        //|       sumExpr SEMI!            { System.out.println ( "I tipi sono differenti" ); }
        
        |       expr PLUS expr{
                            System.err.println ( "I tipi sono differenti" ); 
                          }
...
primaryExpr
        :       i1:ID{
        if(symbolTable.lookupNameInCurrentScope(#i1.getText())==null)
        throw new SemanticException("Variabile non dichiarata",#i1.getText(),#i1.lineNum);
            //    {
            //      System.out.println("Variabile "+#i1.getText()+" non definita");
            //    }
